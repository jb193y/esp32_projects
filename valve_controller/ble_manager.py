# ble_manager.py (Valve Controller)
import bluetooth
import ujson
import machine
import time
...
def get_advertising_payload(name="Valve_Setup"):
    flags = b'\x02\x01\x06'
...
def start_advertising():
    ...
    client_id = cfg.get("client", {}).get("id", "valve_node_01")
...
def update_device_config(data):
    ...
    if "parent_mac" in data:
        cfg["parent"]["mac"] = data["parent_mac"]
    ...
    print("✅ Valve configuration saved. Rebooting device in 2 seconds...")

def start_provisioning():
    global ble_instance, write_handle, read_handle, pending_config
    print("🚀 Initializing BLE Provisioning Service on Valve Node...")
    try:
        ...
        dev_info = {
            "device_id": client_cfg.get("id", "valve_node_01"),
            "device_type": client_cfg.get("type", "valve"),
            "mac": mac_str,
            "fw_ver": client_cfg.get("firmware_version", "valve_v1.0.0")
        }
